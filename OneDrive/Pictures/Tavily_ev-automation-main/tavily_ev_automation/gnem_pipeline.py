...
def ensure_columns(df: pd.DataFrame, preferred: list[str]) -> pd.DataFrame:
    ...
    trailing = [c for c in df.columns if c not in existing]
...
def run_rag_filtering_pipeline(
    *,
    ...
    card_df = stage2_all_df[stage2_all_df["Document_Candidate"] == 1].copy()
    ...
    for row in card_rows:
        ...
        row["Hybrid_Pass"] = 1 if float(row.get("Hybrid_Score", 0.0)) >= hybrid_threshold else 0
    ...
    for row in card_rows:
        ...
        preliminary_shortlist_pass = (
            ...
            and float(row.get("Hybrid_Score", 0.0)) >= hybrid_threshold
            and float(row.get("Direct_Usecase_Score", 0.0)) >= float(args.direct_usecase_threshold)
        )
    ...
    final_rows = apply_diversity_pass(final_rows, max_share=float(args.diversity_max_share))
    final_rows = sorted(
        final_rows,
        key=lambda item: (
            decision_order.get(as_text(item.get("Final_Decision")), 9),
    ...
    curated_rows = [row for row in final_rows if as_text(row.get("Final_Decision")) in {"keep", "review"}]
    curated_rows = copy_curated_documents(curated_rows, out_dir / "final_docs")
    curated_lookup = {as_text(row.get("Candidate_ID")): row for row in curated_rows}
    for idx, row in enumerate(final_rows):
        curated = curated_lookup.get(as_text(row.get("Candidate_ID")))
        if curated:
            final_rows[idx] = curated
        else:
            final_rows[idx]["Curated_Copy_Status"] = "not_curated"
            final_rows[idx]["Curated_File_Path"] = ""
    final_df = pd.DataFrame([{k: v for k, v in row.items() if not str(k).startswith("_")} for row in final_rows])
    final_df = ensure_columns(
        ...
        preferred=[
            ...
            "Embedding_Query_Similarity",
            "Embedding_Rerank_Score",
            ...
            "Duplicate_Master_Flag",
            "Duplicate_Of_Candidate_ID",
            "Duplicate_Group_Type",
            "Near_Duplicate_Flag",
            "Near_Duplicate_Master_Flag",
            "Near_Duplicate_Of_Candidate_ID",
            "Near_Duplicate_Min_Hamming",
            "Near_Duplicate_Text_Similarity",
            "Near_Duplicate_Title_Similarity",
            "URL_Canonical",
            "Query_Mode",
    ...
    curated_document_df = document_registry_dataframe([row for row in final_rows if as_text(row.get("Final_Decision")) in {"keep", "review"}])
    curated_chunk_df = chunk_registry_dataframe([row for row in final_rows if as_text(row.get("Final_Decision")) in {"keep", "review"}])
    processing_log_df = processing_log_dataframe(final_rows)
    ...
    report = {
        ...
        "final_keep_count": int(sum(1 for row in card_rows if as_text(row.get("Final_Decision")) == "keep")),
        "final_review_count": int(sum(1 for row in card_rows if as_text(row.get("Final_Decision")) == "review")),
        "final_discard_count": int(sum(1 for row in card_rows if as_text(row.get("Final_Decision")) == "discard")),
        ...
        "thresholds": {
            ...
            "embedding_backend": embedding_runtime.backend_name if embedding_runtime else "hashing_bow",
        },
        "grounding_files": {
...
def main() -> int:
    ...
    # Stage 2: metadata threshold + PDF candidate + acquisition
    s2 = stage1_dedup_df.copy()
    if "Specificity_Hits" not in s2.columns:
        s2["Specificity_Hits"] = 0
    if "Metadata_TwoSignal_Rule" not in s2.columns:
    ...
    s2["Metadata_Pass"] = (s2["Metadata_Score"] >= float(args.metadata_threshold)).astype(int)
    s2["PDF_Candidate"] = s2["PDF_Candidate"].astype(int)
    if args.disable_metadata_rule_gate:
        s2["Stage2_Filter_Pass"] = ((s2["Metadata_Pass"] == 1) & (s2["PDF_Candidate"] == 1)).astype(int)
    else:
        s2["Stage2_Filter_Pass"] = (
            (s2["Metadata_Pass"] == 1) & (s2["Metadata_Rule_Pass"] == 1) & (s2["PDF_Candidate"] == 1)
        ).astype(int)
    s2 = s2.sort_values(by=["Metadata_Score"], ascending=False).reset_index(drop=True)
    stage2_pool_path = out_dir / "stage2_scored_pool.xlsx"
    write_excel(s2, stage2_pool_path)
    print(f"Stage 2 scored pool: {len(s2)} -> {stage2_pool_path}")

    s2 = s2[s2["Stage2_Filter_Pass"] == 1].copy()
    s2 = s2.sort_values(by=["Metadata_Score"], ascending=False).reset_index(drop=True)
    if args.metadata_target_ratio and args.metadata_target_ratio > 0:
        target_n = max(1, math.ceil(len(stage1_dedup_df) * float(args.metadata_target_ratio)))
        if len(s2) > target_n:
            s2 = s2.head(target_n).copy()
        print(f"Stage 2 ratio cap: {target_n} (ratio={args.metadata_target_ratio})")
    if args.sample_size and args.sample_size > 0:
        s2 = s2.head(args.sample_size).copy()

    local_idx = build_local_pdf_index([Path(p) for p in args.local_pdf_dir if p])
    print(f"Local PDF index size: {len(local_idx)}")
    download_dir = out_dir / "stage2_downloads"

    acquired_rows: list[dict] = []
    for i, row in enumerate(s2.to_dict(orient="records"), start=1):
        if i % 25 == 0 or i == 1:
            print(f"[stage2 {i}/{len(s2)}] acquiring PDF")
        ...
        # 1) local mapping
        local_hit = ""
        for name in candidate_local_names(working):
            p = local_idx.get(name.lower())
            if p:
                local_hit = p
                break
        if local_hit:
            working["Acquisition_Status"] = "LocalFound"
            working["Acquired_File_Path"] = local_hit
            try:
                working["Acquired_Size_MB"] = round(Path(local_hit).stat().st_size / (1024 * 1024), 2)
            except OSError:
                working["Acquired_Size_MB"] = 0.0
            acquired_rows.append(working)
            continue
        ...
        # 2) direct download
        url = str(working.get("URL") or "")
        if not url:
            acquired_rows.append(working)
            continue

        fallback_name = sanitize_filename(
            str(working.get("Filename") or working.get("Title") or working.get("Candidate_ID") or "candidate")
        )
        if not fallback_name.lower().endswith(".pdf"):
            fallback_name = f"{os.path.splitext(fallback_name)[0]}.pdf"

        dl_path, dl_mb, dl_status = download_pdf(
            url=url,
            output_dir=download_dir,
            fallback_name=fallback_name,
            timeout_sec=args.timeout,
    ...
    stage2_df = pd.DataFrame(acquired_rows)
    stage2_selected_path = out_dir / "stage2_selected_by_metadata.xlsx"
    write_excel(stage2_df, stage2_selected_path)
    print(f"Stage 2 selected rows: {len(stage2_df)} -> {stage2_selected_path}")

    if stage2_df.empty:
        stage2_ready_df = stage2_df.copy()
    else:
        stage2_ready_df = stage2_df[
            stage2_df["Acquired_File_Path"].astype(str).str.strip().ne("")
            & stage2_df["Acquired_File_Path"].astype(str).str.lower().str.endswith(".pdf")
        ].copy()
    stage2_ready_path = out_dir / "stage2_acquired_pdfs.xlsx"
    write_excel(stage2_ready_df, stage2_ready_path)
    print(f"Stage 2 acquired PDFs: {len(stage2_ready_df)} -> {stage2_ready_path}")

    # Stage 3: first-page extraction and scoring
    stage3_rows: list[dict] = []
    for i, row in enumerate(stage2_ready_df.to_dict(orient="records"), start=1):
        if i % 25 == 0 or i == 1:
            print(f"[stage3 {i}/{len(stage2_ready_df)}] first-page validation")

        out_row = dict(row)
        pdf_path = str(out_row.get("Acquired_File_Path") or "")
        try:
            fp_text = extract_first_page_text(pdf_path)
            fp_summary_rule = first_page_summary(fp_text)
            fp_score_rule, fp_details = first_page_score(
                first_page_text=fp_text,
                first_page_summary_text=fp_summary_rule,
                golden_summary=golden_summary,
                metadata_score=float(out_row.get("Metadata_Score", 0.0)),
            )
            llm_result: dict[str, Any] | None = None
            if llm_cfg:
                llm_result = llm_first_page_assessment(
                    cfg=llm_cfg,
                    title=as_text(out_row.get("Title")),
                    source_domain=as_text(out_row.get("Source_Domain")),
                    metadata_score=float(out_row.get("Metadata_Score", 0.0)),
                    golden_summary=golden_summary,
                    first_page_text=fp_text,
                )
            fp_score_blended, fp_blend_details = blended_first_page_outcome(
                rule_score=fp_score_rule,
                rule_details=fp_details,
                llm_result=llm_result,
                llm_weight=float(llm_cfg.first_page_weight) if llm_cfg else 0.0,
            )
            fp_summary_final = as_text(fp_blend_details.get("FirstPage_LLM_Summary")) or fp_summary_rule

            out_row["FirstPage_Text_Chars"] = len(fp_text)
            out_row["FirstPage_Summary_Rule"] = fp_summary_rule
            out_row["FirstPage_Summary"] = fp_summary_final
            out_row.update(fp_details)
            out_row.update(fp_blend_details)
            out_row["FirstPage_Score"] = fp_score_blended

            rule_signal = int(fp_details.get("FirstPage_TwoSignal_Rule", 0))
            rule_qspec = int(fp_details.get("FirstPage_QuestionSpecifics_Rule", 0))
            llm_signal = 1 if int(fp_blend_details.get("FirstPage_LLM_DigitalTwinSignal_Count", 0)) >= 2 else 0
            llm_qspec = int(fp_blend_details.get("FirstPage_LLM_QuestionSpecifics_Rule", 0))
            out_row["DigitalTwin_2Signal_Rule"] = 1 if (rule_signal == 1 or llm_signal == 1) else 0
            out_row["Question_With_Specifics_Rule"] = 1 if (rule_qspec == 1 or llm_qspec == 1) else 0
            out_row["Rule_Pass"] = 1 if (
                out_row["DigitalTwin_2Signal_Rule"] == 1 or out_row["Question_With_Specifics_Rule"] == 1
            ) else 0
            final_score = round(
                0.45 * float(out_row.get("Metadata_Score", 0.0)) + 0.55 * float(out_row.get("FirstPage_Score", 0.0)),
                2,
            )
            out_row["Final_Score"] = final_score
            out_row["FirstPage_Pass"] = 1 if float(out_row["FirstPage_Score"]) >= float(args.first_page_threshold) else 0
            out_row["Final_Pass"] = 1 if (
                out_row["Rule_Pass"] == 1
                and out_row["FirstPage_Pass"] == 1
                and final_score >= float(args.final_threshold)
            ) else 0
            out_row["Stage3_Status"] = "Scored"
        except Exception as exc:
            out_row["FirstPage_Text_Chars"] = 0
            out_row["FirstPage_Summary_Rule"] = ""
            out_row["FirstPage_Summary"] = ""
            out_row["FirstPage_Category_Count"] = 0
            out_row["FirstPage_Question_Coverage"] = 0
            out_row["FirstPage_Entity_Count"] = 0
            out_row["FirstPage_Score_Rule"] = 0.0
            out_row["FirstPage_Score_LLM"] = ""
            out_row["FirstPage_Used_LLM"] = 0
            out_row["FirstPage_LLM_Status"] = "failed"
            out_row["DigitalTwin_2Signal_Rule"] = 0
            out_row["Question_With_Specifics_Rule"] = 0
            out_row["FirstPage_Score"] = 0.0
            out_row["FirstPage_Pass"] = 0
            out_row["Final_Score"] = float(out_row.get("Metadata_Score", 0.0))
            out_row["Final_Pass"] = 0
            out_row["Rule_Pass"] = 0
            out_row["Stage3_Status"] = f"Failed:{exc}"
        stage3_rows.append(out_row)
    ...
    for col, default in {
        ...
        if col not in stage3_df.columns:
            stage3_df[col] = default
    stage3_df["Final_Selected_By_Ratio"] = 0
    final_df = stage3_df[stage3_df["Final_Pass"] == 1].copy()
    if args.final_target_ratio and args.final_target_ratio > 0 and len(stage3_df) > 0:
        target_final_n = max(1, math.ceil(len(stage3_df) * float(args.final_target_ratio)))
        final_df = final_df.sort_values(by=["Final_Score"], ascending=False).head(target_final_n).copy()
        stage3_df.loc[final_df.index, "Final_Selected_By_Ratio"] = 1
        print(f"Final ratio cap: {target_final_n} (ratio={args.final_target_ratio})")
    else:
        stage3_df.loc[final_df.index, "Final_Selected_By_Ratio"] = 1

    stage3_scored_path = out_dir / "stage3_first_page_scored.xlsx"
    write_excel(stage3_df, stage3_scored_path)
    print(f"Stage 3 scored: {len(stage3_df)} -> {stage3_scored_path}")

    final_df = final_df.sort_values(by=["Final_Score"], ascending=False).copy()
    if not final_df.empty:
        final_df["_Dedup_Key"] = final_df.apply(
            lambda r: normalize_space(
                " ".join(
                    [
                        as_text(r.get("URL")) or "",
                        canonical_filename_key(as_text(r.get("Filename"))),
                        normalize_space(as_text(r.get("Title"))).lower(),
                    ]
                )
            ),
            axis=1,
        )
        final_df = final_df.drop_duplicates(subset=["_Dedup_Key"], keep="first").drop(columns=["_Dedup_Key"])
    ...
    final_df = ensure_columns(
        ...
        preferred=[
            ...
            "Title",
            "Metadata_Score",
            "FirstPage_Score_Rule",
            "FirstPage_Score_LLM",
            "FirstPage_Score",
            "Final_Score",
            "Final_Selected_By_Ratio",
            "FirstPage_Used_LLM",
            "FirstPage_LLM_Status",
            "FirstPage_LLM_Recommendation",
            "FirstPage_LLM_Confidence",
            "Signal_Categories_Hit",
            "FirstPage_Category_Hits",
            "Acquired_File_Path",
            "Acquisition_Status",
            "Source_Domain",
            "Query",
    ...
    final_path = out_dir / "stage3_final_selected_pdfs.xlsx"
    write_excel(final_df, final_path)
    print(f"Final selected PDFs: {len(final_df)} -> {final_path}")
    ...
    report = {
        ...
        "stage1_all_candidates": int(len(stage1_df)),
        "stage1_dedup_candidates": int(len(stage1_dedup_df)),
        "stage2_scored_pool_count": int(len(stage1_dedup_df)),
        "stage2_selected_by_metadata_pdf": int(len(stage2_df)),
        "stage2_acquired_pdf_count": int(len(stage2_ready_df)),
        "stage3_scored_pdf_count": int(len(stage3_df)),
        "stage3_final_selected_pdf_count": int(len(final_df)),
        "thresholds": {
            "metadata_threshold": float(args.metadata_threshold),
            "first_page_threshold": float(args.first_page_threshold),
            "final_threshold": float(args.final_threshold),
            "metadata_target_ratio": float(args.metadata_target_ratio),
            "final_target_ratio": float(args.final_target_ratio),
            "llm_provider": args.llm_provider,
            "llm_model": args.llm_model,
            "llm_first_page_weight": float(args.llm_first_page_weight),
        },
    }
    report_path = out_dir / "pipeline_report.json"
    report_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    print(textwrap.dedent(
        ...
        Pipeline complete.
          Stage1 all:      {report['stage1_all_candidates']}
          Stage1 dedup:    {report['stage1_dedup_candidates']}
          Stage2 selected: {report['stage2_selected_by_metadata_pdf']}
          Stage2 acquired: {report['stage2_acquired_pdf_count']}
          Stage3 scored:   {report['stage3_scored_pdf_count']}
          Final selected:  {report['stage3_final_selected_pdf_count']}
        """
    ).strip())
    return 0
