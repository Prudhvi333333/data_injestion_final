...
def score_document_card(card: dict[str, Any], grounding: GroundingDictionaries, metadata_score: float) -> dict[str, Any]:
    ...
    evidence_depth_score = min(
        100.0,
        (20.0 if as_text(card.get("Metadata_Summary")) else 0.0)
        + (20.0 if as_text(card.get("FirstPage_Summary")) else 0.0)
        ...
        + (10.0 if as_text(card.get("Evidence_Page_Numbers")) else 0.0)
        + (10.0 if as_text(card.get("Publication_Date")) or int(card.get("Document_Content_Chars", 0)) > 1500 else 0.0),
    )
...
def assess_document_credibility(row: dict[str, Any]) -> dict[str, Any]:
    ...
    evidence_pct = min(
        100.0,
        (20.0 if as_text(row.get("Metadata_Summary")) else 0.0)
        + (15.0 if as_text(row.get("FirstPage_Summary")) else 0.0)
        + (15.0 if as_text(row.get("FirstTwoPages_Summary")) else 0.0)
        + (15.0 if as_text(row.get("Sampled_Page_Summaries")) else 0.0)
        + (15.0 if as_text(row.get("Top_Relevant_Chunks")) else 0.0)
        + (10.0 if as_text(row.get("Evidence_Page_Numbers")) else 0.0)
    ...
    doc_type_text = normalize_space(
        " ".join([as_text(row.get("Title")), as_text(row.get("Metadata_Summary")), as_text(row.get("File_Type"))])
    ).lower()
    doc_type_hits = sum(1 for keyword in DOCUMENT_QUALITY_KEYWORDS if keyword in doc_type_text)
    doc_type_pct = min(100.0, 25.0 + 15.0 * doc_type_hits + (15.0 if as_text(row.get("File_Type")).upper() == "PDF" else 0.0))
